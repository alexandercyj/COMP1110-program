...
UserType promptUserType() {
    ...
    // Handle empty input (default to adult)
    if (input.empty()) {
...
SearchCriteria promptSearchCriteria() {
    ...
    // Handle empty input (default to fastest)
    if (input.empty()) {
    ...
    // Check numeric input
    if (input == "1") return SearchCriteria::FASTEST;
    if (input == "2") return SearchCriteria::CHEAPEST;
    if (input == "3") return SearchCriteria::FEWEST_TRANSFERS;
...
std::string promptStopId(const TransportNetwork& network, const std::string& prompt) {
    std::cout << "  " << prompt << " (ID or English name): ";
    std::string input;
    std::getline(std::cin, input);
    
    std::string stopId = getStopIdFromInput(network, input);
    
    if (stopId.empty() && !input.empty()) {
        std::cout << "  Stop not found: '" << input << "'. Please try again.\n";
    }
